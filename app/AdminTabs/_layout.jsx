...
const AdminTabs = () => {
    ...
    return (
        <>
            {/* Custom Header */}
            {navigation && navigation.getState() && navigation.getState().routes[navigation.getState().index].name === 'Home' && (
                <Header title="Home" icon="home"  />
            )}
            {navigation && navigation.getState() && navigation.getState().routes[navigation.getState().index].name === 'Events' && (
                <Header title="Events" icon="calendar" />
            )}
            {navigation && navigation.getState() && navigation.getState().routes[navigation.getState().index].name === 'Users' && (
                <Header title="Users" icon="people" />
            )}
            {navigation && navigation.getState() && navigation.getState().routes[navigation.getState().index].name === 'Profile' && (
...
const styles = StyleSheet.create({
    headerContainer: {
        ...
        height: 60, // Consistent height with tab bar
    },
    headerTitle: {
        color: 'white',
        fontSize: 18,
