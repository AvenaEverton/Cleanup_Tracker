...
const NotificationsScreen = () => {
  ...
  const fetchNotifications = async (currentUserId) => {
    ...
    try {
      const response = await fetch(
        `http://192.168.1.22:5000/getNotifications?userId=${currentUserId}`
      );
  ...
  const fetchEventDetails = async (eventId) => {
    try {
      const response = await fetch(`http://192.168.1.22:5000/events/${eventId}`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  const handleJoinEvent = async (eventId) => {
    ...
    setIsJoining(true);
    try {
      const response = await fetch(`http://192.168.1.22:5000/joinEvent`, {
        method: "POST",
        headers: {
